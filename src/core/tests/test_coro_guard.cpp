...
auto coro_guard_test(std::vector<int>& vec) -> ms::Task<void> {
  co_await ms::coro_guard(coro_push_back(vec, 1));
  co_await coro_push_back(vec, 2);
}

auto coro_guard_test_stopped(std::vector<int>& vec) -> ms::Task<void> {
  co_await ms::coro_guard(coro_push_back(vec, 1));
  co_await coro_push_back(vec, 2);
...
auto coro_guard_test_exception(std::vector<int>& vec) -> ms::Task<void> {
  co_await ms::coro_guard(coro_push_back(vec, 1));
  co_await coro_push_back(vec, 2);
...
auto test_coro_guard() -> void {
  std::vector<int> vec;
  ms::sync_wait(coro_guard_test(vec));
  assert((vec == std::vector<int>{2, 1}));
}

auto test_coro_guard_stopped() -> void {
  std::vector<int> vec;
  bool success = ms::sync_wait(coro_guard_test_stopped(vec));
  assert(!success);
  assert((vec == std::vector<int>{2, 1}));
}
...
auto test_coro_guard_exception() -> void {
  ...
  try {
    ms::sync_wait(coro_guard_test_exception(vec));
