...
void test_construct_observable()
{
    ...
    int counter = 0;
    auto handler = [&](ms::IoTask<int>&&) noexcept -> ms::IoTask<void> {
        ++counter;
        co_return;
    };
    ms::IoTask<void> task = std::move(obs).subscribe(std::move(handler));
    assert(ms::sync_wait(std::move(task)));
    assert(counter == 0);
}

auto coro_just(int value) -> ms::IoTask<int>
{
    co_return value;
}
...
void test_construct_single_observable()
{
    ...
    int counter = 0;
    auto handler = [&](ms::IoTask<int> task) noexcept -> ms::IoTask<void> {
