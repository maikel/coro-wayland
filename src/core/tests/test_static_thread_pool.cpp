...
void test_construct_and_destroy() { cw::StaticThreadPool pool{1}; }
...
void test_schedule_bulk_one_worker() {
  ...
  auto bulkSender = pool.schedule_bulk(count, [&](std::size_t) -> cw::Task<void> {
...
void test_schedule_bulk_four_workers() {
  cw::StaticThreadPool pool{4, cw::BwosParams{8, 32}};
  std::size_t count = 1'000'000;
  std::vector<float> data(count, 1.0f);
  std::atomic<std::size_t> counter{0};
  auto bulkSender = pool.schedule_bulk(count, [&](std::size_t i) -> cw::Task<void> {
    counter.fetch_add(1, std::memory_order_relaxed);
    co_return;
  });
  cw::sync_wait(bulkSender);
...
void test_schedule_four_workers() {
  ...
  for (std::size_t i = 0; i < count; ++i)
    scope.spawn([](cw::StaticThreadPool* pool, std::atomic<std::size_t>& counter,
                   std::vector<float>& data, std::size_t i) -> cw::Task<void> {
