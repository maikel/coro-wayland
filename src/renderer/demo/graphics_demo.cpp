...
void write_ppm(const std::string& filename, const PixelBufferOwned& buffer) {
  ...
  for (std::size_t y = 0; y < buffer.height(); ++y) {
    for (std::size_t x = 0; x < buffer.width(); ++x) {
      ...
      file << static_cast<int>(color.r) << " " << static_cast<int>(color.g) << " "
...
void test_patterns() {
  ...
  for (int i = 0; i < 8; ++i) {
    ...
    Point end{center.x + static_cast<int>(radius * std::cos(angle)),
  ...
  for (int i = 1; i <= 5; ++i) {
    ...
    Color rect_color{static_cast<std::uint8_t>(50 * i), static_cast<std::uint8_t>(255 - 30 * i),
...
void test_thick_lines() {
  ...
  for (int thickness = 1; thickness <= 10; ++thickness) {
    ...
    Rasterizer::draw_thick_line(span, {50, y_offset}, {350, y_offset}, thickness, colors::GREEN);
  }
...
void performance_test() {
  ...
  for (int i = 0; i < num_lines; ++i) {
    Point start{static_cast<int>(rand() % buffer.width()),
                static_cast<int>(rand() % buffer.height())};
    Point end{static_cast<int>(rand() % buffer.width()),
              static_cast<int>(rand() % buffer.height())};
    Color color{static_cast<std::uint8_t>(rand() % 256), static_cast<std::uint8_t>(rand() % 256),
...
int main() {
  ...
  try {
    ...
    std::cout
        << "Generated PPM files can be viewed with image viewers or converted to other formats.\n";
